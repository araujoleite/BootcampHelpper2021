class ApenasNumImp {
    ...
    menor(): string{
        ...
        for(let i = 0; i < this.lista.length; i++){
            if(this.lista[i] < menor){menor = this.lista[i]}; // Calcula se o número é o menor
        }
    ...
    maior(){
        let maior = 0;
        for(let i = 0; i < this.lista.length; i++){
            if(this.lista[i] > maior){maior = this.lista[i]}; // Calcula se o número é o maior
        }
...
let listaImp: ApenasNumImp = new ApenasNumImp([5, 9.8, "Alan", "José", 1000, 0.45, "Brasil"])
