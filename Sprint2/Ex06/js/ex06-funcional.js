...
class Biografias {
    ...
    atualiza(qualId, name, bio) {
        let index = this.bios.findIndex(person => person.id === qualId); // Busca a ID na lista e recebe o INDEX
    ...
    deleta(qualId) {
        const newListDel = this.bios.filter(person => person.id !== qualId); // Gera uma nova lista, não incluindo a ID deletada
        return newListDel;
    }
    ;
}
// Cria uma lista referência utilizando a organização PESSOA
const listaBase = [{ id: 1, name: "Ada Lovelace", bio: "Ada Lovelace, foi uma matemática e escritora inglesa reconhecida por ter escrito o primeiro algoritmo para ser processado por uma máquina" },
