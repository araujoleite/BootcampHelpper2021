...
let consultaId = 1; //Id que será usado para o teste
function pegaNomeBio(qualId) {
...
*/
function atualiza(qualId, name, bio) {
    let index = lista.findIndex(person => person.id === qualId); // Busca a ID na lista e recebe o INDEX
...
function deleta(qualId) {
    const newListDel = lista.filter(person => person.id !== qualId);
    return newListDel;
}
;
