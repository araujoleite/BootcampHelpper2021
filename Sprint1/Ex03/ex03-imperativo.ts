const list: Array<any> = [5, 9.8, "Alan", "José", 1000, 0.45, "Brasil"]

function retorna(lista: any) {  
    let maior = 0;
    let menor = 0;
    let media = 0;
    const newLista: number[] = [];
    for(let i = 0; i < lista.length; i++){
        if(isFinite(lista[i])){ // Identifica se é um número
            newLista.push(lista[i]); // Inclui o número na array
            if(lista[i] > maior){maior = lista[i]}; // Calcula se o número é o maior
            // if(lista[i] < menor){menor = lista[i]}; // Calcula se o número é o menor
            menor = lista.sort((a: number,b: number) => a - b)[0]
